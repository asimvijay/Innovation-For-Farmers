...
Config = {
  "apiKey": "AIzaSyCekcLOfZ6WTBQk1_ln0MCHDv3QfkkhS1g",
  "authDomain": "crop-monitoring-fc3e3.firebaseapp.com",
  "databaseURL": "https://crop-monitoring-fc3e3-default-rtdb.firebaseio.com",
  "projectId": "crop-monitoring-fc3e3",
  "storageBucket": "gs://crop-monitoring-fc3e3.appspot.com",
  "messagingSenderId": "1015028768867",
  "appId": "1:1015028768867:web:86ec52abfac0e8902e9856",
  "measurementId": "G-8FLQ5GZESN",
}
...
def dashboard():
    if 'user_id' in session:
        user_id_token = session['user_id']  # Retrieve the user ID token from the session

        try:
            # Fetch username from the Realtime Database using the user's ID
            user_info = db.child('users').child(user_id_token).child('user_info').get().val()
            if user_info:
                username = user_info.get('username')
                return render_template('dashboard.html', username=username) # Passing username to the template
            else:
...
def code():
    ...
    with open('msavi.csv', mode='a+') as csvfile:
            csvfile.truncate(0)
    global polygon
    ee.Initialize(project='ee-ewe222vijay')
...
if __name__ == '__main__':
    app.run(debug=True)
# https://console.cloud.google.com/apis/api/earthengine.googleapis.com/metrics?project=optimal-chimera-415809
#https://developers.google.com/earth-engine/cloud/earthengine_cloud_project_setup
#https://developers.google.com/earth-engine/guides/getstarted
    # Web links to create a api  